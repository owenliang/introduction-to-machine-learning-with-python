...
每一个隐层包含多个隐单元，每个隐单元是由前一层的特征经过线性计算后，应用一个非线性函数得到的。

同一个隐层内的所有隐单元作为下一层的特征输入，如此往复。
...
默认神经网络只有1个隐层，这个隐层包含100个隐节点，因此模型需要学习100个线性函数的权重。

减少隐节点个数可以减少训练的计算量，降低过拟合：
...
继续增强L2正则惩罚也可以提升泛化，但实际效果下降了（后面说原因）：
...
# 训练神经网络
mlp = MLPClassifier(solver='lbfgs', alpha=2, hidden_layer_sizes=[10], random_state=0).fit(X_train, y_train)
...
增加隐层的个数，或者修改activation非线性函数，可以使模型更复杂，拟合更高：
...
# 训练神经网络
mlp = MLPClassifier(solver='lbfgs', activation='tanh', hidden_layer_sizes=[10, 10], random_state=0).fit(X_train, y_train)
...
# 减去平均值，然后乘以标准差的倒数, 如此运算之后每个特征的所有样本的mean=0，std=1
X_train_scaled = (X_train - mean_on_train) / std_on_train 
# 对测试集做相同的变换(使用训练集的平均值和标准差) 
X_test_scaled = (X_test - mean_on_train) / std_on_train

# 训练
mlp = MLPClassifier(random_state=0, alpha=1, hidden_layer_sizes=[1000, 1000], max_iter=10000)
